...
namespace av {
    ...
    VideoFrame
    VideoDecoderContext::decode(const Packet &packet, size_t offset, size_t &decodedBytes, OptionalErrorCode ec,
                                bool autoAllocateFrame) {
        return decodeVideo(ec, packet, offset, &decodedBytes, autoAllocateFrame);
    }

    VideoFrame
    VideoDecoderContext::decodeVideo(OptionalErrorCode ec, const Packet &packet, size_t offset, size_t *decodedBytes,
                                     bool autoAllocateFrame) {
        clear_if(ec);
    ...
    void CodecContext2::setCodec(const Codec &codec, bool resetDefaults, Direction direction, AVMediaType type,
    ...
    AVMediaType CodecContext2::codecType(AVMediaType contextType) const noexcept {
        if (isValid()) {
            if (m_raw->codec && (m_raw->codec_type != m_raw->codec->type || m_raw->codec_type != contextType))
                fflog(AV_LOG_ERROR,
                      "Non-consistent AVCodecContext::codec_type and AVCodec::type and/or context type\n");
    ...
    std::pair<int, const error_category *>
    CodecContext2::decodeCommon(AVFrame *outFrame, const Packet &inPacket, size_t offset, int &frameFinished,
    ...
    std::pair<int, const error_category *>
    CodecContext2::encodeCommon(Packet &outPacket, const AVFrame *inFrame, int &gotPacket,
